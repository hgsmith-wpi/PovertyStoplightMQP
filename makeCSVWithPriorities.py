...
#get priorities with follow-up surveys
for file in csv_files:
    df = pd.read_excel(file, sheet_name="Priorities")
    surveys = countSecondSurveys(df)
    for survey in surveys:
        data = data.append(df[df['familyCode'] == survey])

#make csv with priority data
data.to_csv('familyCodes-Priorities.csv', index=False)